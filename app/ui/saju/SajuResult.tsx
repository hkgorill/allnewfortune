...
export default function SajuResult({ result, onReset }: SajuResultProps) {
  ...
  // 사주 기둥(Pillar) 컴포넌트
  const PillarCard = ({ title, pillar, delay }: { title: string; pillar: SajuPillar; delay: number }) => (
    <motion.div
    ...
    >
      ...
      <div className="bg-white/10 border border-white/20 rounded-xl p-2 w-full aspect-[2/3] flex flex-col items-center justify-center gap-2 backdrop-blur-sm">
        {/* 천간 */}
        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold bg-black/20 ${pillar.gan.color}`}>
          {pillar.gan.hanja}
        </div>
        {/* 지지 */}
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl font-bold bg-black/20 ${pillar.ji.color}`}>
          {pillar.ji.hanja}
        </div>
      </div>
  ...
  // 해석 카드 컴포넌트
  const InterpretationCard = ({ icon: Icon, title, content, colorClass, delay }: any) => (
    <motion.div
    ...
    >
      ...
      <div>
        ...
        <p className="text-white/80 text-sm leading-relaxed break-keep">{content}</p>
      </div>
    </motion.div>
  );
    ...
    <div className="w-full max-w-md mx-auto pb-20 relative z-10">
      ...
      <div className="mx-4 mb-8 p-6 bg-gradient-to-b from-gray-900 to-black rounded-[2rem] border border-white/20 shadow-2xl">
         ...
         <div className="mt-6 pt-4 border-t border-white/10">
            <span className="text-xs text-white/50 block text-center mb-3">오행 분포 (목화토금수)</span>
            <div className="flex justify-between px-2">
               <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 rounded-full bg-green-500/20 text-green-400 flex items-center justify-center text-xs border border-green-500/30">{result.fiveElements.wood}</div>
                  <span className="text-[10px] text-green-400">목</span>
               </div>
               <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 rounded-full bg-red-500/20 text-red-400 flex items-center justify-center text-xs border border-red-500/30">{result.fiveElements.fire}</div>
                  <span className="text-[10px] text-red-400">화</span>
               </div>
               <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center justify-center text-xs border border-yellow-500/30">{result.fiveElements.earth}</div>
                  <span className="text-[10px] text-yellow-400">토</span>
               </div>
               <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 rounded-full bg-gray-200/20 text-gray-200 flex items-center justify-center text-xs border border-gray-200/30">{result.fiveElements.metal}</div>
                  <span className="text-[10px] text-gray-200">금</span>
               </div>
               <div className="flex flex-col items-center gap-1">
                  <div className="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center text-xs border border-blue-500/30">{result.fiveElements.water}</div>
      ...
      <div className="mx-4 space-y-3 mb-8">
        <InterpretationCard 
          icon={User} title="타고난 성향" content={result.interpretation.personality} 
          colorClass="text-emerald-400" delay={0.5} 
        />
        <InterpretationCard 
          icon={Coins} title="재물운" content={result.interpretation.wealth} 
          colorClass="text-yellow-400" delay={0.6} 
        />
        <InterpretationCard 
          icon={Briefcase} title="직업운" content={result.interpretation.career} 
          colorClass="text-blue-400" delay={0.7} 
        />
        <InterpretationCard 
          icon={Heart} title="연애/애정운" content={result.interpretation.love} 
          colorClass="text-pink-400" delay={0.8} 
        />
      </div>
